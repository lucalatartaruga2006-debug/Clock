...
export default function App() {
  ...
  return (
    <div className="w-screen relative overflow-hidden bg-black font-clock" style={{ height: '100dvh' }}>
      ...
      {phase === 'powerup' && (
        <div className="absolute inset-0 bg-black/85 flex flex-col items-center justify-center p-4 sm:p-6 overflow-auto">
          <h2 className="font-horror text-blood text-2xl sm:text-4xl mb-2 tracking-wider">POTENZIAMENTO</h2>
          <p className="text-bone/60 text-xs sm:text-sm mb-4 text-center">Scegli un potere. L'ora {hour} incombe.</p>
          <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 justify-center max-w-3xl w-full">
      ...
      {phase === 'merchant' && (
        <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center p-4 sm:p-6 overflow-auto">
          <h2 className="font-horror text-purple-400 text-2xl sm:text-4xl mb-2 tracking-wider">L'OMBRA MERCANTE</h2>
          <p className="text-bone/60 text-xs sm:text-sm mb-2 text-center px-2">Un'ombra incappucciata ti osserva. Offre tre doni. Costo: {MERCHANT_COST} secondi ciascuno.</p>
          <p className="text-purple-300/70 text-xs mb-4 text-center">Questi acquisti valgono solo per questa partita.</p>
          <div className="text-gold text-lg mb-4">⏳ {currency}s</div>
          <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 justify-center max-w-3xl w-full">
      ...
      {phase === 'shop' && (
        <div className="absolute inset-0 bg-black/95 flex flex-col items-center p-4 sm:p-6 overflow-auto">
          <div className="flex items-center justify-between w-full max-w-5xl mb-4 shrink-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 w-full max-w-5xl">
            {ALL_UPGRADES.map((def) => (
              <ShopCard key={def.category} def={def} player={player} onBuy={() => buyUpgrade(def.category)} />
            ))}
          </div>
          <button onClick={() => setPhase('menu')}
            className="mt-6 bg-zinc-800 hover:bg-zinc-700 text-bone px-6 py-3 rounded-lg font-clock tracking-widest border border-bone/20 shrink-0">← Torna al menu</button>
        </div>
      )}
